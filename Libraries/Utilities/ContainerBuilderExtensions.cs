...
namespace Autofac
{
    /* Custom ILifetimeScope builder to register the placeholder IHttpContextAccessor implementation. This is necessary because
     * IHttpContextAccessor is an application-wide dependency, not even a shell singleton. This would cause all types of failures
     * on other shells when calling IHttpContextAccessor.Set() (e.g. to work around work context being lost because of async
     * or building a new WorkContextScope), since that would set the HttpContext everywhere.
     * This could be much better if this is fixed: https://github.com/OrchardCMS/Orchard/issues/4338 as now we need a stub HttpContext
     * to carry the work context.

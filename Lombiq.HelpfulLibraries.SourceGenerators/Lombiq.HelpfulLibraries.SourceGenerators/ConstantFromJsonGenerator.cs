...
{{
    ...
    public void Initialize(IncrementalGeneratorInitializationContext context)
    {
        ...
        context.RegisterSourceOutput(namesAndContents.Collect(), (ctx, contents) =>
        {
    ...
    private static (ClassDeclarationSyntax, bool reportAttributeFound, List<Dictionary<string, string>>) GetClassDeclarationForSourceGen(
    ...
    /// <param name="classDeclarations">Nodes annotated with the [ConstantFromJson] attribute that trigger the
    /// generate action.</param>
        ...
        ImmutableArray<(ClassDeclarationSyntax, List<Dictionary<string, string>>)> classDeclarations)
    {
        ...
        foreach (var (classDeclarationSyntax, attributeData) in classDeclarations)
        {
            ...
            foreach (var dictionary in attributeData)
            {
                ...
                var constantName = dictionary["constantName"];
                var fileName = dictionary["fileName"];
                var jsonPath = dictionary["jsonPath"];
                
                // Try get content of file from dictionary where key ends with filename
                var fileContent =_fileContents
                    .FirstOrDefault(kvp => 
                        kvp.Key.EndsWith(fileName.Replace("\"", "")));
...
";
    ...
    private JsonElement? FindProperty(JsonElement element, string propertyName)
    {

...
public class ManualConnectingIndexServiceTests(ManualConnectingIndexServiceFixture fixture) : IClassFixture<ManualConnectingIndexServiceFixture>
{
    public const string NamePrefix = "test_";
    private static readonly int[] Numbers = [3, 6];

    [Fact]
    public Task IndicesShouldHaveMatchingDocuments() => fixture.SessionAsync(async session =>
    {
    ...
    [Fact]
    public Task AllIndexShouldRetrieveItsDocument() => fixture.SessionAsync(async session =>
    {
        ...
        documents.Select(document => document.Name)
            .ShouldBe(fixture.Documents.Where((_, index) => index is not 3 and not 6).Select(document => document.Name));
    });

    [Fact]
    public Task MissingOrDeletedIndexShouldNotRetrieveAnyDocument() => fixture.SessionAsync(async session =>
    {
